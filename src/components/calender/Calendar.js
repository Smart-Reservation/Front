...
function dateLTnow(date,now){
  if(date<=now){
    return date.getFullYear()===now.getFullYear()&&date.getMonth()===now.getMonth()?date.getDate()<now.getDate():true;
  }
  return false;
}
  ...
  return (
    <>
      ...
      <CalendarContainer>
        ...
        <WeekContainer>
          {WEEKDAYS.map((value) => <Day key={value} date={value} onClick={handleEmptyClick} />)}
        </WeekContainer>
        {fullMonth}
      </CalendarContainer>
    </>
